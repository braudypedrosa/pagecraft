...
export function installActionFeedback(css = ACTION_FEEDBACK_CSS) {
  ...
  const begin = (button, message, key, options = {}) => {
    ...
    const iconOnly=button&&(button.matches('.iconbtn,.mdel,.pc-entry-delete-icon,[data-icon-only]')||(!button.textContent.trim()&&button.hasAttribute('aria-label')));
