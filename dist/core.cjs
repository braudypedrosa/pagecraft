...
var slotGet = (n, s) => s.i < 0 ? n.props[s.prop] : n.props[s.prop][s.i][s.sub];
var slotSet = (n, s, v) => {
  if (s.i < 0) n.props[s.prop] = v;
  else n.props[s.prop][s.i][s.sub] = v;
};
...
function replaceAll(q, to, o = {}) {
  ...
  state.pages.forEach((p) => {
    PAGE_TEXT.forEach(([k]) => {
      if (typeof p[k] === "string") p[k] = swap2(p[k]);
    });
    scan(p.tree);
  });
  scan(state.footer);
...
function migrate(d) {
  ...
  if (v < 5) {
    ...
    ((d.meta.tokens || {}).text || []).forEach((t) => {
      if (!t.tag && TAGS[t.id]) t.tag = TAGS[t.id];
    });
  }
  if (v < 6) {
    d.meta = d.meta || {};
