...
function renderNode(n, o) {
  ...
  switch (n.type) {
    ...
    case "slider": {
      ...
      return `<div class="pagecraft-slider-box" data-slider>${track}` + btn("p", "Previous slides") + btn("n", "Next slides") + '<div class="pagecraft-slider-dots" data-slide-dots role="group" aria-label="Choose a slide" hidden></div></div>';
    }
    case "list": {
    ...
    case "image": {
      ...
      const set = o.variants && !o.edit && /^asset:[A-Za-z0-9][A-Za-z0-9._:-]*$/.test(String(p.src || "")) ? imageWidths(p.w).map((w) => `${p.src}@${w} ${w}w`) : [];
