...
const stayCard = (stay: Stay) => {
  const node = N('box', { layout: 'block', tag: 'article', link: `${stay.slug}.html` }, { d: { 'background-color': C('paper'), 'border-width': '1px', 'border-style': 'solid', 'border-color': C('line'), overflow: 'hidden', 'min-height': '100%' }, t: {}, m: {} }, [
    image(asset(stay.assetId), stay.alt, { height: '330px' }, { height: '250px' }),
  ...
  node.st = { hover: { d: { transform: 'translateY(-4px)', 'border-color': C('ink') }, t: {}, m: {} }, focus: { d: { outline: `3px solid ${C('signal')}`, 'outline-offset': '3px' }, t: {}, m: {} } };
  return node;
};
...
const header = () => [named(N('section', { tag: 'header' }, {
  ...
  column(20, [button('Plan a stay', 'contact.html', 'dark')], { 'align-items': 'flex-end' }, { display: 'none' })
...
const darkCta = (title: string, copy: string) => section([row([
  ...
  column(34, [button('Start an inquiry', 'contact.html', 'light')], { 'align-items': 'flex-end', 'justify-content': 'flex-end' })
], { 'align-items': 'flex-end', gap: '48px' })], { 'background-color': C('ink'), color: C('paper') });
...
const home = (): Page => ({
  id: 'page-home', name: 'Home', slug: 'index', title: 'Marea — A quieter way to stay by the coast', desc: 'A fictional collection of four coastal vacation rentals, designed as a fully editable Pagecraft template.', ogImage: asset('coastal-living-room'), tree: [
    named(section([row([
      column(43, [
        reveal(heading('A quieter way to stay by the coast.', 'display', 'h1', { d: { 'font-family': Core.stackFor('Newsreader', 'f'), 'font-size': 'clamp(50px,5.3vw,70px)', 'font-weight': '500', 'line-height': '.98', 'letter-spacing': '-.045em', 'max-width': '12ch' }, t: {}, m: { 'font-size': '43px' } })),
        reveal(text('<p>Marea is a fictional collection of four distinct homes shaped around salt air, long meals, and unhurried days.</p>', 'lead', { d: { color: C('muted'), 'max-width': '42ch', 'margin-top': '27px' }, t: {}, m: {} }), '.08s'),
        reveal(row([column(0, [button('Explore the stays', 'stays.html', 'dark')]), column(0, [button('How Marea works', 'about.html', 'line')])], { 'margin-top': '32px', gap: '12px' }), '.16s')
      ]),
      column(57, [reveal(image(asset('coastal-living-room'), 'Limewashed coastal living room opening to a sea-view terrace', { height: '650px' }, { height: '420px' }, false), '.1s')])
    ], { 'align-items': 'flex-end', gap: '68px' })], { 'background-color': C('paper'), padding: '76px 32px 112px' }, { padding: '52px 18px 68px' }), 'marea-hero'),
    ...
    section([
      ...
      N('box', { layout: 'grid' }, { d: { 'grid-template-columns': 'repeat(3,minmax(0,1fr))', gap: '0', 'margin-top': '52px', 'border-top-width': '1px', 'border-top-style': 'solid', 'border-top-color': C('line') }, t: {}, m: { 'grid-template-columns': '1fr', 'margin-top': '36px' } }, [
        ['Arrive with a plan', 'Clear directions, entry notes, and the first essentials in one message.'], ['Set the kitchen', 'Shape an optional pantry list around the pace of the stay.'], ['Find the quiet route', 'Share the paths, coves, and tables that fit the guest’s day.']
      ].map(([title, copy], index) => N('box', { layout: 'block' }, { d: { padding: '30px', 'border-bottom-width': '1px', 'border-bottom-style': 'solid', 'border-bottom-color': C('line'), ...(index ? { 'border-left-width': '1px', 'border-left-style': 'solid', 'border-left-color': C('line') } : {}) }, t: {}, m: { padding: '26px 0', 'border-left-width': '0px' } }, [heading(title, 'subtitle', 'h3'), text(`<p>${copy}</p>`, 'body', { d: { color: C('muted'), 'margin-top': '12px', 'max-width': '33ch' }, t: {}, m: {} })])))
