...
import { buildTemplateDocument } from '../premade-sites/salt-house/1.0.0/source.ts';
...
it('exposes an actual native image picker and leaves typography connected to global settings',()=>{
 const d=buildTemplateDocument(),component=d.meta.components!.find(c=>c.id==='salt-property')!;
 expect(component.props.find(p=>p.k==='image')?.t).toBe('img');
 for(const id of ['display','title','subtitle'])expect(d.meta.tokens!.text.find(t=>t.id===id)?.css.d['font-family']).toBe('inherit');
...
it.each([1,3,7])('preserves %i independent project plates and omits missing imagery',count=>{
 const d=buildTemplateDocument();const parent=all(d.pages[0].tree).find(n=>n.children.some(c=>c.use==='salt-property'))!;const base=parent.children[0];
 parent.children=Array.from({length:count},(_,i)=>{const n=structuredClone(base);n.id=`redesign-stress-${i}`;n.vals={...n.vals,title:`Independent ${i} — a much longer holiday home name`,description:i%2?'Short.':'Daylight, enclosure and a shared threshold are tested in this deliberately longer project description.',...(i===0?{image:''}:{})};return n;});
...
it('keeps explicit spacing stable when a native save canonicalizes CSS key order',()=>{
 ...
 const hero=d.pages[0].tree[0];expect(hero.css.d['padding-top']).toBe('40px');expect(all(hero.children).filter(n=>n.type==='image')).toHaveLength(3);
 ...
 for(const output of [original,sorted])expect(output).toContain('padding-top:40px;');
});
