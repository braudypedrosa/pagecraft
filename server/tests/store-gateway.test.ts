...
import {
  GatewayAssetStore, GatewayAuthStore, GatewayConnectedStore, GatewayStore, PagecraftGateway
...
const fakeGateway = (answer: (call: Call) => unknown) => {
  ...
  return {
    calls,
    gateway: new PagecraftGateway('https://gateway.invalid/', 'test-key', request as typeof fetch)
  };
};
...
test('gateway asset listing is metadata-only while id/path reads carry only one body', async () => {
  ...
  const meta = { id: 'a1', site_id: 's1', name: 'Photo.png', type: 'image/png', w: 10, h: 20 };
  const { gateway, calls } = fakeGateway(call => {
    if (call.op === 'asset.list') return [meta];
    if (call.op === 'asset.get' || call.op === 'asset.byPath') return { ...meta, bytes };
  ...
  a.deepEqual([...(await assets.byPath('s1', 'assets/photo-a1.png'))!.bytes], [1, 2, 250]);
  const put = await assets.put({
    siteId: 's1', name: 'Photo.png', type: 'image/png', w: 10, h: 20,
    bytes: new Uint8Array([1, 2, 250])
  });
  a.equal('bytes' in put, false, 'upload responses do not echo the image through the gateway');
  a.equal(await assets.remove('s1', 'a1'), true);
  a.deepEqual(calls.map(call => call.op), [
    'asset.list', 'asset.get', 'asset.byPath',
    'asset.blob.putChunk', 'asset.putBlob', 'asset.remove'
  ]);
  a.equal((calls[4].args.asset as Record<string, unknown>).name, 'Photo.png',
    'the human-readable name remains intact');
});

test('gateway connected asset finalization carries its connection guard', async () => {
  const meta = { id: 'wp-a1', site_id: 's1', name: 'Photo.png', type: 'image/png', w: 10, h: 20 };
  const { gateway, calls } = fakeGateway(call => {
    if (call.op === 'asset.blob.putChunk') return { stored: true };
  ...
  const saved = await assets.putConnected({
    id: 'wp-a1', siteId: 's1', name: 'Photo.png', type: 'image/png', w: 10, h: 20,
    bytes: new Uint8Array([1, 2, 250])
  }, 'connection-production');
  a.equal(saved?.id, 'wp-a1');
  a.deepEqual(calls.map(call => call.op), ['asset.blob.putChunk', 'asset.putBlobConnected']);
...
test('gateway site metadata excludes documents and hot public lookups are boundedly cached', async () => {
  ...
  const { gateway, calls } = fakeGateway(call => {
    ...
    if (call.op === 'site.bySlug') { slugReads++; return siteRow; }
    throw new Error(`unexpected ${call.op}`);
  });
  ...
  a.equal((await sites.bySlug('acme'))!.version, 3);
  a.equal(slugReads, 1, 'a hot public path does not call a multi-second gateway twice');
...
test('gateway publish carries the release sequence required by the database CAS', async () => {
  ...
  const sites = new GatewayStore(gateway);
  a.equal((await sites.publish('s1', 2, 'release-two', 2))?.publishedReleaseId, 'release-two');
  a.deepEqual(calls[0], {
    op: 'site.publish', args: { id: 's1', version: 2, releaseId: 'release-two', releaseSequence: 2 }
  });
});

test('gateway CMS write heads preserve the per-connection sequence ledger contract', async () => {
  const head = {
    connectionId: 'production', collectionId: 'posts', itemId: 'one', writeSequence: 9,
    idempotencyKey: 'cms-write-0009', bodyHash: '9'.repeat(64), version: 12
  };
  const { gateway, calls } = fakeGateway(call => {
  ...
  const sites = new GatewayStore(gateway);
  a.deepEqual(await sites.cmsWriteHeads('s1', 'production', [cmsItemKey('posts', 'one')]), [head]);
  a.deepEqual(calls[0], {
    op: 'site.cmsWriteHeads',
    args: { id: 's1', connectionId: 'production', itemKeys: [cmsItemKey('posts', 'one')] }
  });
});

test('gateway connection revocation preserves the scoped idempotent result contract', async () => {
  const row = {
    id: 'connection-1', site_id: 's1', created_by: 'u1', installation_id: 'install-1',
    environment: 'production', profile: 'existing-theme', target_origin: 'https://wp.test', target_path: '/',
    redirect_uri: 'https://wp.test/wp-admin/admin.php?page=pagecraft',
    webhook_url: 'https://wp.test/wp-json/pagecraft/v1/releases/available', scopes: ['release:read'],
    status: 'revoked', code_challenge: 'x'.repeat(43), authorization_code_digest: 'code',
    authorization_code_expires_at: '2030-01-01T00:00:00.000Z', authorization_code_used_at: null,
    confirmation_expires_at: null, confirmed_at: '2026-08-26T00:00:00.000Z',
    access_token_digest: 'access', access_token_expires_at: '2030-01-01T00:00:00.000Z',
    refresh_token_digest: null, desired_release_id: null, pending_release_id: null,
    next_sequence: 1, last_acknowledged_sequence: 0, active_release_id: 'frozen-release',
    active_hash: 'f'.repeat(64), revoked_at: '2026-08-26T00:00:00.000Z',
    revocation_idempotency_key: 'disconnect-key', created_at: '2026-08-26T00:00:00.000Z',
    updated_at: '2026-08-26T00:00:00.000Z'
  };
  const { gateway, calls } = fakeGateway(call => {
    if (call.op === 'connection.revoke') return { ok: true, connection: row, alreadyRevoked: true };
    throw new Error(`unexpected ${call.op}`);
  });
  const connected = new GatewayConnectedStore(gateway);
  const result = await connected.revokeConnection({
    id: 'connection-1', accessTokenDigest: 'access', refreshTokenDigest: 'refresh',
...
test('gateway reads the persisted production canonical independently of active connection listings', async () => {
  const row = {
    id: 'production-old', site_id: 's1', created_by: 'u1', installation_id: 'install-1',
    environment: 'production', profile: 'existing-theme', target_origin: 'https://canonical.wp.test',
    target_path: '/site/', redirect_uri: 'https://canonical.wp.test/wp-admin/',
    webhook_url: 'https://canonical.wp.test/wp-json/pagecraft/v1/releases/available', scopes: [],
    status: 'revoked', code_challenge: 'x'.repeat(43), authorization_code_digest: 'code',
    authorization_code_expires_at: '2030-01-01T00:00:00.000Z', authorization_code_used_at: null,
    confirmation_expires_at: null, confirmed_at: '2026-08-26T00:00:00.000Z',
    access_token_digest: 'access', access_token_expires_at: '2030-01-01T00:00:00.000Z',
    refresh_token_digest: null, desired_release_id: null, pending_release_id: null,
    next_sequence: 1, last_acknowledged_sequence: 0, active_release_id: null, active_hash: null,
    revoked_at: '2026-08-26T00:00:00.000Z', revocation_idempotency_key: 'disconnect',
  ...
  a.deepEqual((await connected.connectionHistoryForSite('s1')).map(item => item.id), ['production-old'],
    'release safety can retain a revoked target scope without exposing it as active');
  a.deepEqual(calls, [{
    op: 'connection.canonicalProduction', args: { siteId: 's1' }
  }, {
    op: 'connection.historyForSite', args: { siteId: 's1' }
  }]);
});

test('gateway preserves the two-phase provision and confirmation contract', async () => {
  const base = {
    id: 'connection-two-phase', site_id: 's1', created_by: 'u1', installation_id: 'install-two-phase',
    environment: 'staging', profile: 'existing-theme', target_origin: 'https://staging.wp.test',
    target_path: '/', redirect_uri: 'https://staging.wp.test/wp-admin/',
    webhook_url: 'https://staging.wp.test/wp-json/pagecraft/v1/releases/available', scopes: [],
    status: 'provisioned', code_challenge: 'x'.repeat(43), authorization_code_digest: 'code',
    authorization_code_expires_at: '2026-08-26T00:10:00.000Z',
    authorization_code_used_at: '2026-08-26T00:01:00.000Z',
    confirmation_expires_at: '2026-08-26T00:31:00.000Z', confirmed_at: null,
    access_token_digest: 'access', access_token_expires_at: '2026-08-26T00:30:00.000Z',
    refresh_token_digest: 'refresh', desired_release_id: null, pending_release_id: null,
    next_sequence: 1, last_acknowledged_sequence: 0, active_release_id: null, active_hash: null,
    revoked_at: null, revocation_idempotency_key: null,
    created_at: '2026-08-26T00:00:00.000Z', updated_at: '2026-08-26T00:01:00.000Z'
  };
  const { gateway, calls } = fakeGateway(call => {
    if (call.op === 'connection.provision') return base;
    if (call.op === 'connection.confirm') {
      return {
        connection: { ...base, status: 'active', confirmed_at: '2026-08-26T00:02:00.000Z' },
  ...
  const connected = new GatewayConnectedStore(gateway);
  a.equal((await connected.provisionConnection(base.id, {
    accessTokenDigest: 'access', accessTokenExpiresAt: '2026-08-26T00:30:00.000Z',
    refreshTokenDigest: 'refresh', confirmationExpiresAt: '2026-08-26T00:31:00.000Z'
  }))?.status, 'provisioned');
  const confirmed = await connected.confirmConnection({
    id: base.id, accessTokenDigest: 'access', installationId: base.installation_id,
  ...
  a.deepEqual(calls.map(call => call.op), ['connection.provision', 'connection.confirm']);
});

test('gateway preserves the atomic native WordPress content snapshot contract', async () => {
  const snapshot = {
    connectionId: 'connection-1', generation: 3, bodyHash: 'a'.repeat(64),
    items: [{
      id: 'wp:page:2', objectType: 'page' as const, title: 'About',
      url: 'https://wp.test/about/', modifiedAt: '2026-08-26T01:00:00.000Z'
    }], syncedAt: '2026-08-26T02:00:00.000Z'
  };
  const wire = {
    connection_id: snapshot.connectionId, generation: snapshot.generation,
    body_hash: snapshot.bodyHash, items: snapshot.items, synced_at: snapshot.syncedAt
  };
  const { gateway, calls } = fakeGateway(call => {
    if (call.op === 'contentIndex.replace') return { ok: true, snapshot: wire, duplicate: false };
    if (call.op === 'contentIndex.forSite') return [wire];
    throw new Error(`unexpected ${call.op}`);
  });
  const connected = new GatewayConnectedStore(gateway);
  a.deepEqual(await connected.replaceWordPressContentIndex(snapshot), {
    ok: true, snapshot, duplicate: false
  });
...
test('gateway records the hosted publication finalization before release delivery traversal', async () => {
  ...
  const connected = new GatewayConnectedStore(gateway);
  a.deepEqual(await connected.commitReleasePublication({
    siteId: 's1', releaseId: 'release-1', sourceVersion: 4, releaseSequence: 2,
    publishedAt: '2026-08-26T00:00:00.000Z'
  }, async () => { throw new Error('the gateway must coordinate this remotely'); }), {
    publishedVersion: 4, publishedReleaseId: 'release-1'
  });
  a.equal(await connected.markReleasePublished(
    'release-1', '2026-08-26T00:00:00.000Z'
  ), true);
  a.deepEqual(calls, [{
    op: 'release.commitPublication', args: {
      siteId: 's1', releaseId: 'release-1', sourceVersion: 4, releaseSequence: 2,
      publishedAt: '2026-08-26T00:00:00.000Z'
    }
  }, {
    op: 'release.markPublished', args: {
      releaseId: 'release-1', publishedAt: '2026-08-26T00:00:00.000Z'
    }
  }]);
});
...
test('gateway preserves the Edge wrong-hash rollback verdict without local mutation', async () => {
  const { gateway, calls } = fakeGateway(call => {
    if (call.op === 'deployment.record') return { ok: false, error: 'wrong-hash' };
    throw new Error(`unexpected ${call.op}`);
  });
  const connected = new GatewayConnectedStore(gateway);
  const input = {
    connectionId: 'production', releaseId: 'release-new', sequence: 3,
    status: 'rolled_back' as const, activeHash: '9'.repeat(64), error: null,
    detail: { stage: 'rolled_back' }, idempotencyKey: 'unknown-rollback', bodyHash: '8'.repeat(64)
  };
...
test('gateway auth resolves a session and all memberships with one operation each', async () => {
  const { gateway, calls } = fakeGateway(call => {
    ...
    if (call.op === 'auth.userForSession') return { id: 'u1', email: 'me@acme.test', name: 'Me' };
  ...
  const auth = new GatewayAuthStore(gateway);
  a.deepEqual((await auth.usersByIds(['u1', 'u1'])).map(user => user.id), ['u1']);
...
test('a malformed successful gateway response fails closed', async () => {
  ...
  const gateway = new PagecraftGateway('https://gateway.invalid', 'test-key', request as typeof fetch);
...
test('gateway client refuses an oversized JSON control request before network I/O', async () => {
  let requested = false;
  const request = async () => { requested = true; return Response.json({ data: true }); };
  const gateway = new PagecraftGateway('https://gateway.invalid', 'test-key', request as typeof fetch);
  await a.rejects(() => gateway.call('oversized', { value: 'x'.repeat(16 * 1024 * 1024) }), /too large/);
