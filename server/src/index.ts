...
import {
  GatewayOwnedSiteStore,
  MemoryOwnedSiteStore,
  type OwnedSiteStore,
  PgOwnedSiteStore,
} from "./accounts.ts";
...
async function pickStores(): Promise<{
  store: Store;
  assets: AssetStore;
  auth: AuthStore;
  connected: ConnectedStore;
  owned: OwnedSiteStore;
  ...
  if (gatewayUrl || gatewayKey) {
    if (!gatewayUrl || !gatewayKey) {
      throw new Error(
        "DATABASE_GATEWAY_URL and DATABASE_GATEWAY_KEY must be set together",
      );
    }
    const {
      PagecraftGateway,
    ...
    const cloudMutations = new GatewayHostedPublishPreparer(
      gateway,
      store,
      assets,
    );
    return {
      ...
      owned: new GatewayOwnedSiteStore(gateway, store),
  ...
  if (!url) {
    if (process.env.NODE_ENV === "production") {
      throw new Error(
        "production requires DATABASE_GATEWAY_URL or DATABASE_URL; refusing ephemeral memory storage",
      );
    }
    console.warn(
  ...
  const { PgStore, PgAssetStore, PgAuthStore, PgConnectedStore } = await import(
    "./store-pg.ts"
  );
...
if (!supabaseUrl || !supabaseKey || !turnstileSiteKey) {
  throw new Error(
    "account auth requires SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY and TURNSTILE_SITE_KEY",
  );
}
if (production && testAuth) {
  throw new Error("PAGECRAFT_AUTH_TEST_MODE is forbidden in production");
}
...
const challenge = testAuth
  ? new TestHumanChallenge()
  : new SupabaseHumanChallenge();
...
function releaseSecurity(): {
  ...
  if (envelopeJson && envelopeBase64) {
    throw new Error(
      "set only PAGECRAFT_KEYSET_ENVELOPE_BASE64URL or the legacy PAGECRAFT_KEYSET_ENVELOPE, not both",
    );
  }
  if (
    envelopeBase64 && (!/^[A-Za-z0-9_-]+$/.test(envelopeBase64) ||
      Buffer.from(envelopeBase64, "base64url").toString("base64url") !==
        envelopeBase64)
  ) {
  ...
  const supplied =
    [privateWire, keyId, envelopeWire, rootWire].filter(Boolean).length;
  if (!supplied) {
    console.warn(
      "Connected WordPress signing is not provisioned — release endpoints fail closed.",
    );
    return {};
  }
  if (supplied !== 4) {
    throw new Error(
      "PAGECRAFT_RELEASE_PRIVATE_KEY, PAGECRAFT_RELEASE_KEY_ID, a keyset envelope, and PAGECRAFT_ROOT_PUBLIC_KEY must be set together",
    );
  }
  let envelope: KeysetEnvelopeV1;
  try {
    envelope = JSON.parse(envelopeWire!) as KeysetEnvelopeV1;
  } catch (error) {
    ...
    const reason = error instanceof Error
      ...
      : "parse failed";
    throw new Error(
      `PAGECRAFT_KEYSET_ENVELOPE is not valid JSON (length ${
        envelopeWire!.length
      }; boundary code points ${first}/${last}; ${reason})`,
    );
  }
  const privateKey = createPrivateKey({
    key: Buffer.from(privateWire!, "base64url"),
    format: "der",
    type: "pkcs8",
  });
  const keyset = verifyKeysetEnvelope({
    envelope,
    rootPublicKey: keyFromRawPublic(rootWire!),
  });
  const active = keyset.keys.find((key) => key.id === keyId);
  if (
    !active || active.publicKey !== rawPublicKey(createPublicKey(privateKey))
  ) {
    throw new Error(
      "release private key does not match its root-signed keyset entry",
    );
  }
  return {
    releaseSigning: { keyId: keyId!, privateKey },
    keysetEnvelope: envelope,
  };
}
...
function packageRegistry() {
  ...
  if (!signing.releaseSigning || !signing.keysetEnvelope) {
    throw new Error(
      "signed package updates require provisioned release signing",
    );
  }
  ...
  for (const item of configured) {
    if (!item.path || !item.version) {
      throw new Error(
        `${item.slug} package path and version must be set together`,
      );
    }
    if (!existsSync(item.path)) {
      throw new Error(`${item.slug} archive does not exist: ${item.path}`);
    }
    registry.add({
      slug: item.slug,
      version: item.version,
      bytes: new Uint8Array(readFileSync(item.path)),
      generatedAt: statSync(item.path).mtime.toISOString(),
      signing: signing.releaseSigning,
      keysetEnvelope: signing.keysetEnvelope,
    });
  }
  return registry;
}
  ...
  try {
    for (const event of await connected.claimWebhooks(webhookWorker, 10)) {
      ...
      const delay = Math.min(
        6 * 60 * 60 * 1000,
        15_000 * 2 ** Math.min(event.attempts, 8),
      );
    ...
    console.error(
      "WordPress webhook outbox could not be drained:",
...
if (
  !accountAuth && !process.env.DATABASE_URL &&
  !process.env.DATABASE_GATEWAY_URL && !(await store.listMeta()).length
) {
  const Core = await import("../../app/src/core/index.ts");
  Core.seed();
  await store.create({
    host: EDITOR_HOST === "localhost"
      ? "site.localhost"
      : "site." + EDITOR_HOST,
    name: "Demo",
    doc: {
...
if (OWNER) {
  ...
  for (const s of await store.listMeta()) {
    await auth.grant(s.id, user.id, "owner");
  }
  console.log(`owner    ${OWNER} — POST /auth/login for a link`);
} else if (!accountAuth) {
  console.warn(
    "OWNER_EMAIL is not set — nobody can sign in. The sites still serve.",
  );
}
...
if (CLIENT) {
  ...
  for (const s of await store.listMeta()) {
    await auth.grant(s.id, user.id, "content");
  }
...
if (mail) {
  ...
  if (!accountAuth && process.env.NODE_ENV === "production") {
    throw new Error(
      "production requires complete SMTP_HOST, SMTP_USER, SMTP_PASS and MAIL_FROM settings; refusing to log sign-in tokens",
    );
  }
  if (!accountAuth) {
    console.warn(
      "SMTP is not configured — development login links are printed here instead of emailed.",
    );
    console.warn(
...
serve({ fetch: app.fetch, port: PORT }, (info) => {
  ...
  store.listMeta().then((sites) =>
    sites.forEach((s) =>
      console.log(`site     http://${s.host}:${info.port}/  (${s.name})`)
    )
  );
});
