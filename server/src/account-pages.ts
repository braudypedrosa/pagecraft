...
<meta name="color-scheme" content="light"><title>${esc(title)} — Pagecraft</title>
<style>
...
<main class="account"><div class="wordmark">Pagecraft</div><section class="panel" aria-labelledby="title">
<h1 id="title">Sign in to your sites</h1><p>Use the email and password for your Pagecraft account.</p>
${notice(input.error, input.message)}<form class="stack" method="post" action="/auth/login">
...
<p class="foot"><a href="/forgot-password">Forgot password?</a> · <a href="/sign-up">Create account</a></p>
...
export const dashboardPage = (user: User, sites: DashboardSite[], ownerCount: number, error?: string) => {
  ...
  return shell('Sites', `<main class="dashboard"><header class="topbar"><div class="wordmark">Pagecraft</div><div class="actions"><span class="account-email meta-value">${esc(user.email)}</span><form method="post" action="/auth/logout"><button class="secondary" type="submit">Sign out</button></form></div></header>
