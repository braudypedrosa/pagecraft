...
import {
  type ConnectedEditorCredential,
  type ConnectedGrant,
  type ConnectedGrantKind,
  type ConnectedStore,
...
import {
  ...
  type GatewayBlobChunkV1,
  type GatewayBlobDescriptorV1,
  splitGatewayBlob,
  validateGatewayBlobDescriptor,
} from "./gateway-blobs.ts";
import type {
  CloudMutationFastPath,
  CloudPublishResult,
  CloudSaveResult,
  HostedPublishPreparer,
  ManualImportCatalogRead,
  ManualImportProjectRead,
  ManualImportReadFastPath,
} from "./app.ts";

interface GatewayReply<T> {
  data: T;
}
...
export class PagecraftGateway {
  ...
  async call<T>(op: string, args: Record<string, unknown> = {}): Promise<T> {
    ...
    if (!response.ok) {
      throw new GatewayError(
        body.error || `gateway answered ${response.status}`,
        body.code,
      );
    }
...
interface ManualImportCatalogWire {
  authorized: boolean;
  owner_id?: string;
  sites?: SiteRow[];
}

interface ManualImportProjectWire {
  authorized: boolean;
  owner_id?: string;
  site?: SiteRow | null;
}
...
export class GatewayManualImportReader implements ManualImportReadFastPath {
  ...
  async catalog(accessDigest: string): Promise<ManualImportCatalogRead> {
    const read = await this.gateway.call<ManualImportCatalogWire>(
      "manualImport.catalog",
      { digest: accessDigest },
    );
    if (!read.authorized || !read.owner_id) return { authorized: false };
    return {
      authorized: true,
      ownerId: read.owner_id,
      sites: (read.sites || []).map(toSite),
    };
  }
  ...
  async project(
    ...
    const read = await this.gateway.call<ManualImportProjectWire>(
      "manualImport.project",
      { digest: accessDigest, projectId },
    );
    if (!read.authorized || !read.owner_id) return { authorized: false };
    return {
      authorized: true,
      ownerId: read.owner_id,
      site: read.site ? toSite(read.site) : null,
    };
  }
}
...
export class GatewayStore implements Store {
  ...
  async byHost(host: string) {
    ...
    const row = await this.gateway.call<SiteRow | null>("site.byHost", {
      host,
    });
  ...
  async bySlug(slug: string) {
    ...
    if (hit !== undefined) return hit;
    const row = await this.gateway.call<SiteRow | null>("site.bySlug", {
      slug,
    });
    const site = row ? toSite(row) : null;
    this.remember(site);
  ...
  accept(row: SiteRow | Site) {
    const site = "published_version" in row
      ? toSite(row as SiteRow)
      : structuredClone(row as Site);
    this.clearCache();
    this.remember(site);
  ...
  async listMeta() {
    return (await this.gateway.call<SiteMetaRow[]>("site.listMeta")).map(
      (row) => ({
        id: row.id,
        host: row.host,
        slug: row.slug,
  ...
  async create(
    ...
    if (input.slug && !wanted) {
      throw new Error(`not a usable path: ${input.slug}`);
    }
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      try {
        ...
        if (
          wanted || !/slug/i.test(message) || !/unique|duplicate/i.test(message)
        ) throw error;
        taken.push(slug);
      }
    }
  ...
  async setSlug(id: string, slug: string) {
    ...
    try {
      const row = await this.gateway.call<SiteRow | null>("site.setSlug", {
        id,
        slug: wanted,
      });
      const site = row ? toSite(row) : null;
  ...
  async setName(id: string, name: string) {
    ...
    const row = await this.gateway.call<SiteRow | null>("site.setName", {
      id,
      name: clean,
    });
    const site = row ? toSite(row) : null;
  ...
  async setHost(id: string, host: string) {
    try {
      const row = await this.gateway.call<SiteRow | null>("site.setHost", {
        id,
        host,
      });
      const site = row ? toSite(row) : null;
  ...
  async save(
    ...
    const row = await this.gateway.call<SiteRow | null>("site.save", {
      id,
      doc,
      version,
      savedBy,
  ...
  async saveConnectedCms(
    ...
    const result = await this.gateway.call<
      { row: SiteRow | null; guarded: boolean }
    >("site.saveConnectedCms", {
      id,
      doc,
      version,
      savedBy,
    ...
    if (current.version !== version) {
      return {
        ok: false,
        conflict: { yours: version, theirs: current.version },
      };
    }
    return { ok: false, guarded: true };
  }
  async history(id: string) {
    return (await this.gateway.call<RevisionRow[]>("site.history", { id })).map(
      toRevision,
    );
  }
  async revision(id: string, version: number) {
    const row = await this.gateway.call<RevisionRow | null>("site.revision", {
      id,
      version,
    });
  ...
  async cmsWriteHeads(id: string, connectionId: string, itemKeys: string[]) {
    return this.gateway.call<CmsWriteHead[]>("site.cmsWriteHeads", {
      id,
      connectionId,
      itemKeys,
    });
  ...
  async publishHosted(input: {
    ...
    const row = await this.gateway.call<SiteRow | null>(
      "site.publishHosted",
      input,
    );
    const site = row ? toSite(row) : null;
...
interface ConnectionWire {
  ...
  environment: WordPressConnection["environment"];
  ...
  webhook_url: string;
  scopes: string[];
  status: WordPressConnection["status"];
  code_challenge: string;
  authorization_code_digest: string;
  authorization_code_expires_at: string;
  authorization_code_used_at: string | null;
  confirmation_expires_at: string | null;
  confirmed_at: string | null;
  access_token_digest: string | null;
  access_token_expires_at: string | null;
  refresh_token_digest: string | null;
...
const toConnection = (row: ConnectionWire): WordPressConnection => ({
  ...
  authorizationCodeExpiresAt: new Date(row.authorization_code_expires_at)
    .toISOString(),
  authorizationCodeUsedAt: row.authorization_code_used_at
    ? new Date(row.authorization_code_used_at).toISOString()
    : null,
  confirmationExpiresAt: row.confirmation_expires_at
    ? new Date(row.confirmation_expires_at).toISOString()
    : null,
...
interface ReleaseSummaryWire extends
  Omit<
    SiteReleaseSummary,
    | "siteId"
    | "sourceVersion"
  ...
  artifact_blob: GatewayBlobDescriptorV1;
...
interface ReleaseWire extends ReleaseSummaryWire {
...
const checkedBlob = (row: ReleaseSummaryWire) => {
  ...
  if (
    row.artifact_blob.hash !== row.artifact_hash ||
    row.artifact_blob.bytes !== row.artifact_bytes
  ) {
    throw new GatewayError(
      "gateway release blob metadata does not match the release",
      "INVALID_RESPONSE",
    );
  }
...
const toReleaseReservation = (
  row: ReleaseReservationWire,
): ReleaseReservation => ({
  siteId: row.site_id,
  idempotencyKey: row.idempotency_key,
...
const toEditorCredential = (
  row: EditorCredentialWire,
): ConnectedEditorCredential => ({
  digest: row.digest,
  connectionId: row.connection_id,
  siteId: row.site_id,
...
interface WordPressContentIndexWire {
  ...
  body_hash: string;
  items: WordPressContentIndexSnapshot["items"];
  synced_at: string;
}
const toWordPressContentIndex = (
  row: WordPressContentIndexWire,
): WordPressContentIndexSnapshot => ({
  connectionId: row.connection_id,
...
export class GatewayConnectedStore implements ConnectedStore {
  ...
  private async artifactFor(row: ReleaseSummaryWire) {
    ...
    for (let start = 0; start < descriptor.chunkCount; start += 4) {
      const batch = await Promise.all(
        Array.from(
          { length: Math.min(4, descriptor.chunkCount - start) },
          async (_, offset) => {
            const chunk = await this.gateway.call<GatewayBlobChunkV1 | null>(
              "release.artifactChunk",
              {
                id: row.id,
                index: start + offset,
              },
            ...
            if (!chunk) {
              throw new GatewayError(
                "release artifact chunk is unavailable",
                "INVALID_RESPONSE",
              );
            }
            return chunk;
  ...
  async createConnection(
    ...
    return toConnection(
      await this.gateway.call<ConnectionWire>("connection.create", { input }),
    );
  }
  async connection(id: string) {
    const row = await this.gateway.call<ConnectionWire | null>(
      "connection.byId",
      { id },
    );
  ...
  async connectionsForSite(siteId: string) {
    return (await this.gateway.call<ConnectionWire[]>("connection.forSite", {
      siteId,
    })).map(toConnection);
  }
  async connectionHistoryForSite(siteId: string) {
    return (await this.gateway.call<ConnectionWire[]>(
      "connection.historyForSite",
      { siteId },
    )).map(toConnection);
  }
  async canonicalProductionConnection(siteId: string) {
    const row = await this.gateway.call<ConnectionWire | null>(
      "connection.canonicalProduction",
      { siteId },
    );
    return row ? toConnection(row) : null;
  }
  ...
  async consumeGrant(digest: string, kind: ConnectedGrantKind, now: string) {
    const row = await this.gateway.call<GrantWire | null>("grant.consume", {
      digest,
      kind,
      now,
    });
  ...
  async putEditorCredential(
    input: Omit<ConnectedEditorCredential, "createdAt">,
  ) {
    return toEditorCredential(
      await this.gateway.call<EditorCredentialWire>("editorCredential.put", {
        input,
      }),
    );
  }
  async editorCredential(digest: string, now: string) {
    const row = await this.gateway.call<EditorCredentialWire | null>(
      "editorCredential.get",
      { digest, now },
    );
    return row ? toEditorCredential(row) : null;
  }
  async replaceWordPressContentIndex(
    input: WordPressContentIndexSnapshot,
  ): Promise<WordPressContentIndexResult> {
    const result = await this.gateway.call<
    ...
    return result.ok
      ? {
        ok: true,
        snapshot: toWordPressContentIndex(result.snapshot),
        duplicate: result.duplicate,
      }
      : result;
  }
  async wordpressContentIndexesForSite(siteId: string) {
    return (await this.gateway.call<WordPressContentIndexWire[]>(
      "contentIndex.forSite",
      { siteId },
    ))
  ...
  async reserveRelease(
    ...
    return toReleaseReservation(
      await this.gateway.call<ReleaseReservationWire>("release.reserve", {
        input,
      }),
    );
  }
  async createRelease(release: SiteRelease) {
    ...
    if (
      split.descriptor.hash !== release.artifactHash ||
      split.descriptor.bytes !== release.artifactBytes
    ) {
      throw new Error(
        "release artifact does not match its declared hash and size",
      );
    }
    for (let start = 0; start < split.chunks.length; start += 4) {
      await Promise.all(
        split.chunks.slice(start, start + 4).map((chunk) =>
          this.gateway.call("release.blob.putChunk", {
    ...
    const result = await this.gateway.call<
      { row: ReleaseWire; created: boolean }
    >("release.create", {
      release: { ...metadata, artifactBlob: split.descriptor },
    });
    const stored = checkedBlob(result.row);
    if (
      stored.hash !== split.descriptor.hash ||
      stored.bytes !== split.descriptor.bytes
    ) {
      throw new GatewayError(
        "idempotency key already belongs to a different release artifact",
        "IDEMPOTENCY_CONFLICT",
      );
    }
    return {
      release: toRelease(result.row, release.artifact),
      created: result.created,
    };
  }
  async release(id: string) {
    const row = await this.gateway.call<ReleaseWire | null>("release.byId", {
      id,
    });
    return row ? toRelease(row, await this.artifactFor(row)) : null;
  }
  async commitReleasePublication(
    input: {
      siteId: string;
      releaseId: string;
    ...
    _publishHosted: () => Promise<
      { publishedVersion: number; publishedReleaseId: string | null } | null
    >,
  ) {
  ...
  async desiredTarget(connectionId: string) {
    const result = await this.gateway.call<
      {
        connection: ConnectionWire;
        release: ReleaseSummaryWire;
        target: TargetWire;
      } | null
    >("target.desired", { connectionId });
    return result
      ? {
        connection: toConnection(result.connection),
        release: toReleaseSummary(result.release),
        target: toTarget(result.target),
      }
      : null;
  }
  async deploymentsForRelease(releaseId: string) {
    return (await this.gateway.call<DeploymentWire[]>("deployment.forRelease", {
      releaseId,
    })).map(toDeployment);
  }
  async recordDeployment(
    ...
    return {
      ...
      deployment: result.deployment
        ? toDeployment(result.deployment)
        : undefined,
      connection: result.connection
        ? toConnection(result.connection)
        : undefined,
    };
  }
  async enqueueWebhook(
    ...
    return toWebhook(
      await this.gateway.call<WebhookWire>("webhook.enqueue", { input }),
    );
  }
  async claimWebhooks(worker: string, limit: number) {
    return (await this.gateway.call<WebhookWire[]>("webhook.claim", {
      worker,
      limit,
    })).map(toWebhook);
  }
  async settleWebhook(
    ...
    await this.gateway.call<boolean>("webhook.settle", {
      eventId,
      delivered,
      nextAttemptAt,
      error,
...
interface AssetMetaWire {
  ...
  stored_bytes?: number | string | null;
...
const toAsset = (row: AssetWire): Asset => ({
  ...
  w: row.w,
  h: row.h,
  bytes: new Uint8Array(Buffer.from(row.bytes, "base64")),
  ownerId: row.owner_id || undefined,
  storedBytes: row.stored_bytes == null ? undefined : Number(row.stored_bytes),
  originalBytes: row.original_bytes == null
    ? undefined
...
export class GatewayAssetStore implements AssetStore {
  ...
  async list(siteId: string) {
    const hit = this.cachedList(siteId);
    if (hit) return hit;
    const assets =
      (await this.gateway.call<AssetMetaWire[]>("asset.list", { siteId })).map(
        toAssetRecord,
      );
  ...
  cachedList(siteId: string) {
    const hit = this.listed.get(siteId);
    if (!hit || hit.until <= Date.now()) {
      this.listed.delete(siteId);
      return null;
    }
    return structuredClone(hit.assets);
  }
  acceptList(siteId: string, assets: AssetRecord[]) {
    this.listed.set(siteId, {
      until: Date.now() + this.cacheMs,
  ...
  async put(asset: Omit<Asset, "id"> & { id?: string }, quota?: AssetQuota) {
    ...
    for (let start = 0; start < split.chunks.length; start += 4) {
      await Promise.all(
        split.chunks.slice(start, start + 4).map((chunk) =>
          this.gateway.call("asset.blob.putChunk", {
            blob: split.descriptor,
            chunk,
          })
        ),
    ...
    try {
      const row = await this.gateway.call<AssetMetaWire>("asset.putBlob", {
        asset: {
          ...
          originalBytes: quota?.originalBytes,
      ...
      if (
        ...
        throw new AssetQuotaError(
          await this.usage(quota.ownerId, quota.limitBytes),
        );
      }
      throw error;
    }
  ...
  async putConnected(
    ...
    try {
      const row = await this.gateway.call<AssetMetaWire | null>(
  ...
  async usage(ownerId: string, limitBytes = FREE_STORAGE_BYTES) {
    const usedBytes = await this.gateway.call<number>("asset.usage", {
      ownerId,
    });
...
interface AccessWire extends UserWire {
  role: Role | null;
}
...
type HostedPublishPreparationWire =
  | {
    status: "ok";
    user: UserWire;
    role: Role;
    ...
    assets: AssetMetaWire[];
  }
  | { status: "missing" | "forbidden" };

/** One production gateway invocation replaces auth mapping, membership, site, revision, and
 * asset-metadata calls that otherwise cross the shared-host/Supabase boundary serially. */
export class GatewayHostedPublishPreparer
  implements HostedPublishPreparer, CloudMutationFastPath {
  private gateway: PagecraftGateway;
  private store?: GatewayStore;
  private assets?: GatewayAssetStore;
  private owners = new Map<string, { until: number; user: User }>();
  constructor(
    gateway: PagecraftGateway,
    store?: GatewayStore,
    assets?: GatewayAssetStore,
  ) {
    this.gateway = gateway;
    this.store = store;
    this.assets = assets;
  ...
  private ownerKey(authUserId: string, siteId: string) {
    return `${authUserId}\0${siteId}`;
  }
  private rememberOwner(authUserId: string, siteId: string, user: User) {
    this.owners.set(this.ownerKey(authUserId, siteId), {
      until: Date.now() + EDITOR_SOURCE_CACHE_MS,
      user: structuredClone(user),
    });
  }
  private cachedOwner(authUserId: string, siteId: string) {
    const key = this.ownerKey(authUserId, siteId);
    const hit = this.owners.get(key);
    if (!hit || hit.until <= Date.now()) {
      this.owners.delete(key);
      return null;
    }
    return structuredClone(hit.user);
  }

  async prepare(input: Parameters<HostedPublishPreparer["prepare"]>[0]) {
    const prepared = await this.gateway.call<HostedPublishPreparationWire>(
      "site.prepareHostedPublish",
      {
        id: input.siteId,
        newUserId: crypto.randomUUID(),
        authUserId: input.identity.authUserId,
        email: normalEmail(input.identity.email),
        name: input.identity.name.trim(),
      },
    );
    if (prepared.status !== "ok") return prepared;
    if (prepared.role !== "owner") {
      throw new GatewayError(
        "gateway returned an invalid publish role",
        "INVALID_RESPONSE",
      );
    }
    const result = {
      status: "ok" as const,
      user: toUser(prepared.user),
      role: prepared.role,
      site: toSite(prepared.site),
      revision: prepared.revision ? toRevision(prepared.revision) : null,
      assets: prepared.assets.map(toAssetRecord),
    };
    this.store?.accept(result.site);
    this.assets?.acceptList(input.siteId, result.assets);
    this.rememberOwner(input.identity.authUserId, input.siteId, result.user);
    return result;
  }
  ...
  async saveAuthorized(
    input: Parameters<CloudMutationFastPath["saveAuthorized"]>[0],
  ): Promise<CloudSaveResult> {
    const result = await this.gateway.call<
      | { status: "saved"; user: UserWire; role: Role; site: SiteRow }
      | { status: "missing" }
      | { status: "forbidden" }
    ...
    const site = this.store?.accept(result.site) || toSite(result.site);
    if (result.role === "owner") {
      this.rememberOwner(input.identity.authUserId, input.siteId, user);
    }
    return { status: "saved", site };
  }

  cachedPublishSource(
    input: Parameters<CloudMutationFastPath["cachedPublishSource"]>[0],
  ) {
    const user = this.cachedOwner(input.identity.authUserId, input.siteId);
    const site = this.store?.cachedById(input.siteId, input.sourceVersion);
    const assets = this.assets?.cachedList(input.siteId);
    if (!user || !site || !assets) return null;
    return {
      user,
      site,
      revision: {
  ...
  async publishAuthorized(
    input: Parameters<CloudMutationFastPath["publishAuthorized"]>[0],
  ): Promise<CloudPublishResult> {
    const result = await this.gateway.call<
      | { status: "published"; user: UserWire; site: SiteRow }
      | { status: "missing" }
      | { status: "forbidden" }
      | { status: "conflict"; currentVersion: number }
...
export class GatewayAuthStore implements AuthStore {
  ...
  userByEmail(email: string) {
    return this.gateway.call<UserWire | null>("auth.userByEmail", {
      email: normalEmail(email),
    })
      .then((row) => row ? toUser(row) : null);
  }
  userById(id: string) {
    return this.gateway.call<UserWire | null>("auth.userById", { id }).then(
      (row) => row ? toUser(row) : null,
    );
  }
  userByAuthId(authUserId: string) {
    return this.gateway.call<UserWire | null>("auth.userByAuthId", {
      authUserId,
    })
      .then((row) => row ? toUser(row) : null);
  }
  ...
  usersByIds(ids: string[]) {
    return ids.length
      ? this.gateway.call<UserWire[]>("auth.usersByIds", {
        ids: [...new Set(ids)],
  ...
  async useLink(digest: string) {
    const row = await this.gateway.call<
      { email: string; expires_at: string } | null
    >("auth.useLink", { digest });
    if (!row) return null;
  ...
  async sessionByDigest(digest: string) {
    const row = await this.gateway.call<SessionWire | null>(
      "auth.sessionByDigest",
      { digest },
    );
    if (!row) return null;
  ...
  async accessForSession(digest: string, siteId: string) {
    const row = await this.gateway.call<AccessWire | null>(
  ...
  async dropSession(digest: string) {
    await this.gateway.call("auth.dropSession", { digest });
  }
  async membership(siteId: string, userId: string) {
    const row = await this.gateway.call<MembershipWire | null>(
      "auth.membership",
      { siteId, userId },
    );
  ...
  async membershipsForUser(userId: string) {
    return (await this.gateway.call<MembershipWire[]>(
  ...
  async grant(siteId: string, userId: string, role: Role) {
    return toMembership(
      await this.gateway.call<MembershipWire>("auth.grant", {
        siteId,
        userId,
        role,
      }),
  ...
  async members(siteId: string) {
    return (await this.gateway.call<MembershipWire[]>("auth.members", {
      siteId,
    })).map((row) => ({
      ...toMembership(row),
      email: row.email || "",
  ...
  changeMemberRole(siteId: string, userId: string, role: Role) {
    return this.gateway.call<MemberChangeResult>("auth.changeMemberRole", {
      siteId,
      userId,
      role,
    });
  ...
  removeMember(siteId: string, userId: string) {
    return this.gateway.call<MemberRemovalResult>("auth.removeMember", {
      siteId,
      userId,
    });
  }
  async provisionInvitation(input: {
    ...
    return result.status === "granted"
      ? {
        ...
        membership: toMembership(result.membership),
      }
      : result;
  }
  drainInvitationOutbox(worker: string, limit = 10) {
    return this.gateway.call<InvitationDrainResult>(
  ...
  async createManualImportCredential(
    input: Omit<
      ManualImportCredential,
      "status" | "createdAt" | "updatedAt" | "revokedAt"
    >,
  ) {
    return toManualImport(
      await this.gateway.call<ManualImportWire>("auth.manualImport.create", {
        input: {
          ...input,
          accessExpiresAt: new Date(input.accessExpiresAt).toISOString(),
        },
      }),
    );
  }
  ...
  async rotateManualImportAccess(
    id: string,
    digest: string,
    expiresAt: number,
  ) {
